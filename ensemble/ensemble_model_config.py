...
class EnsembleModelConfig:
    ...
    def __init__(self, em_version, is_debug, random_state=39):
        ...
        self.dir_em_output = os.path.join('output', em_version)
        self.is_debug = is_debug
        self.random_state = random_state

        self.cv = StratifiedShuffleSplit(n_splits=5, test_size=0.2,
    ...
    def _load_en_config(self, model_symbol, encoder):
        ...
        if self.is_debug:
            self.dict_parmas_grid = {
                ...
                f'{model_symbol}__fit_intercept': [True],
                f'{model_symbol}__l1_ratio': [0, 1],
                f'{model_symbol}__max_iter': [10]
            }
        else:
            self.dict_parmas_grid = {
                f'{model_symbol}__alpha': [0, 0.2, 0.4, 0.5, 0.75],
                f'{model_symbol}__fit_intercept': [True, False],
                f'{model_symbol}__l1_ratio': [0, 0.2, 0.4, 0.6, 0.8, 1],
    ...
    def _load_rf_config(self, model_symbol, encoder):
        ...
        if self.is_debug:
            self.dict_parmas_grid = {
                ...
                f'{model_symbol}__max_depth': [2],
                f'{model_symbol}__min_samples_split': [2]
            }
        else:
            self.dict_parmas_grid = {
                f'{model_symbol}__n_estimators': [150],
                f'{model_symbol}__criterion': ['squared_error'],
