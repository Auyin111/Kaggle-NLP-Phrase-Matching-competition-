...
def merge_context2(df, cfg):  # A temporary version for me to deal with error in creating translated dataset.
    df_context_grp_1 = pd.read_csv("../own_dataset/df_context_grp_1.csv")
    # form text column
    df.loc[:, 'context_grp_1'] = df.context.apply(lambda x: x[0])
    df = df.merge(df_context_grp_1, how='left', on='context_grp_1')
    assert df.text_grp_1.isnull().sum() == 0, 'some of the context text are missing'
    ...
    if cfg.use_grp_2:
        df_context_grp_2 = pd.read_csv("../own_dataset/df_context_grp_2.csv")
